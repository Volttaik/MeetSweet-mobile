...
export default function CompleteRegistrationScreen() {
  ...
  const handleComplete = async () => {
    ...
    try {
      await register({
        full_name: params.username || 'MeetSweet User',
        ...
        avatar_url: params.avatarUri || undefined,
      });
      router.replace({ pathname: '/verify-email', params: { email: (params.email || '').trim() } });
    } catch (err) {
    ...
    <LinearGradient colors={['#16081E', '#0D0B1A']} style={styles.gradient}>
      ...
      >
        ...
        >
          ...
          {!!serverError && (
            <View style={{ backgroundColor: 'rgba(239,68,68,0.12)', borderRadius: 12, padding: 12, marginBottom: 16 }}>
              <Text style={{ color: '#EF4444', textAlign: 'center', fontSize: 13, fontFamily: 'Poppins_400Regular' }}>{serverError}</Text>
            </View>
          )}
          ...
          >
            ...
            >
              ...
              <Text style={styles.primaryBtnText}>{loading ? 'Creating Account…' : 'Complete Account'}</Text>
            </LinearGradient>
          </TouchableOpacity>
        </ScrollView>
      </View>
