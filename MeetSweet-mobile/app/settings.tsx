...
export default function SettingsScreen() {
  ...
  // ── General: in-app sound effects preference ───────────────────────────────
  const handleSoundsToggle = useCallback(async (value: boolean) => {
    setSoundsEnabled(value);
    await soundService.setEnabled(value);
    if (value) soundService.playSuccess();
    toast.success(value ? 'Sound effects enabled' : 'Sound effects muted');
  }, []);
    ...
    <View style={[styles.bg, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <GradientBorder radius={T.RADIUS.lg} surface={T.SURFACE} style={rs.sectionBorder}>
          ...
          <ToggleRow
            label="Sound Effects"
            sub="Send, receive, and confirmation sounds"
            value={soundsEnabled}
