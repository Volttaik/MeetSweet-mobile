...
export async function getBankDetails(): Promise<BankDetails | null> {
  try {
    const data = await authedRequest<any>('/creator/wallet/bank-details');
    if (data && (data.bank_name || data.bankName)) {
    ...
    // Fall back to stored local copy if network/endpoint unavailable
  }
  try {
    const stored = await AsyncStorage.getItem('@creator_bank_details');
