 ...
 * Shows the quoted message content (text, image, voice, video, document)
 * in a faded/subtle appearance. Tapping the surrounding Pressable (wired in
 * MsChatBubble) scrolls the chat to the original message.
 */
import React from 'react';
import { Image, StyleSheet, Text, View } from 'react-native';
import { ArrowBendUpLeft, FileText, Microphone } from 'phosphor-react-native';
...
export function MsReplyPreviewBubble({ reply, position }: Props) {
  ...
  // Media indicator label/icon when the quoted message has no text preview.
  let mediaIcon: React.ReactNode = null;
  let mediaLabel = '';
  if (!hasImage && !hasAudio) {
    if (!reply.text) {
      mediaLabel = 'Attachment';
      mediaIcon = <FileText size={11} color={isOwn ? 'rgba(255,255,255,0.6)' : T.TEXT_3} />;
    }
  }
      ...
      <View style={styles.body}>
        ...
        {hasImage ? (
          ...
          <View style={styles.mediaRow}>
            {hasAudio ? (
              <Microphone size={11} color={isOwn ? 'rgba(255,255,255,0.6)' : T.TEXT_3} />
            ) : mediaIcon}
            <Text style={[styles.text, isOwn ? styles.textOwn : styles.textOther]} numberOfLines={2}>
              {hasAudio
                ? (reply.text || 'Voice note')
                : (reply.text || mediaLabel || 'Attachment')}
            </Text>
          </View>
        )}
      </View>
