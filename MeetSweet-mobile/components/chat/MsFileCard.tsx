...
export function MsFileCard({ message, position, onPress, onDownload, onLongPress }: Props) {
  ...
  const size     = sizeVal ? formatFileSize(sizeVal) : '';
  const mimeVal  = (message.msMimeType ?? message.mimeType) ?? undefined;
  const nameVal  = (message.msFileName ?? message.fileName) ?? undefined;
  const label    = getFileLabel(mimeVal, nameVal);
