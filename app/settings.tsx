...
async function loadPref<T extends object>(key: string, defaults: T): Promise<T> {
  try {
...
async function savePref<T>(key: string, val: T) {
  await AsyncStorage.setItem(key, JSON.stringify(val)).catch(() => {});
}
...
function UsernameModal({
  ...
  const checkAvailability = (v: string) => {
    ...
    debounceRef.current = setTimeout(() => {
      // Backend GET /users/:username — simulate check
      setChecking(false);
      setAvailable(v.toLowerCase() !== 'taken' && v.length >= 3);
    }, 600);
  };
  ...
  const handleSave = () => {
    if (!available) return;
    toast.info('Username change requires backend implementation');
    onClose();
  };
...
function EmailModal({
  ...
  const handleSave = async () => {
    ...
    setSaving(false);
    toast.info('Email change requires backend implementation.\nDocumented in BACKEND_REQUIRED.md.');
    onClose();
  };
  ...
  return (
    <BottomSheet visible={visible} onClose={onClose} title="Two-Factor Authentication">
      <View style={{ gap: 16, paddingBottom: 4 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: T.SURFACE_2, borderRadius: T.RADIUS.md, padding: 14 }}>
          <Shield size={22} color={T.TEXT_2} />
          <View style={{ flex: 1 }}>
            <Text style={rs.rowLabel}>Authenticator App</Text>
            <Text style={rs.rowSub}>Not configured</Text>
          </View>
          <View style={rs.badge}><Text style={rs.badgeText}>Off</Text></View>
        </View>
        <Text style={[ms.sub, { lineHeight: 20 }]}>
          Two-factor authentication adds an extra layer of security. When enabled, you'll need a code from your authenticator app to sign in.{'\n\n'}
          Full 2FA setup requires backend implementation.
        </Text>
        <TouchableOpacity style={ms.saveBtn} onPress={() => { toast.info('2FA setup requires backend implementation'); onClose(); }} activeOpacity={0.8}>
          <Text style={ms.saveLabel}>Set Up 2FA</Text>
        </TouchableOpacity>
      </View>
    </BottomSheet>
  );
...
function LoginHistoryModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  return (
    <BottomSheet visible={visible} onClose={onClose} title="Login History">
      <View style={{ gap: 12, paddingBottom: 4 }}>
        <View style={[sess.card, { flexDirection: 'column', alignItems: 'flex-start', gap: 4 }]}>
          <Text style={rs.rowLabel}>Today — This Device</Text>
          <Text style={rs.rowSub}>Current session · Active now</Text>
        </View>
        <Text style={[ms.sub, { textAlign: 'center', marginTop: 8 }]}>
          Full login history requires backend implementation.{'\n'}Documented in BACKEND_REQUIRED.md.
        </Text>
        <TouchableOpacity style={ms.cancelBtn} onPress={onClose} activeOpacity={0.7}>
...
export default function SettingsScreen() {
  ...
  return (
    <View style={[styles.bg, { paddingTop: insets.top }]}>
      ...
      >
        ...
        <View style={rs.section}>
          ...
          <Row
            ...
            onPress={() => toast.info('Linked accounts require backend implementation')}
          />
        </View>
        ...
        <View style={rs.section}>
          ...
          <Divider />
          <Row label="Two-Factor Authentication" sub="Add an extra layer of security" badge="Off" onPress={() => setModal('twoFactor')} />
        </View>
        ...
        <View style={rs.section}>
          ...
          <Row
            label="Theme"
            sub="Dark"
            onPress={() => toast.info('Dark theme is active. Light theme coming soon.')}
          />
        </View>
      ...
      />
      <TwoFactorModal
        visible={modal === 'twoFactor'}
        onClose={() => setModal(null)}
      />
      <LoginHistoryModal
        visible={modal === 'loginHistory'}
        onClose={() => setModal(null)}
      />
      ...
      <MsConfirmDialog
        ...
        destructive
        onConfirm={() => { setDeleteConfirm(false); toast.error('Account deletion requires backend implementation'); }}
