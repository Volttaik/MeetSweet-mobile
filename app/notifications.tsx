...
import { ArrowLeft, BellSlash, Check, RefreshCw, Trash } from 'phosphor-react-native';
...
export default function NotificationsScreen() {
  ...
  const handlePress = async (n: Notification) => {
    ...
    // Direct routing based on notification payload
    if (n.chatRoomId || n.type === 'message' || n.type === 'dm') {
      if (n.chatRoomId) {
        router.push(`/chat-room/${n.chatRoomId}`);
        return;
      }
    }

    if (n.type === 'wallet' || n.type === 'payout' || n.type === 'payment') {
      router.push('/wallet');
      return;
    }

    if (n.type === 'subscribe' || n.type === 'creator') {
      if (n.actor?.id) {
        router.push(`/creator/${n.actor.id}`);
        return;
      }
    }
