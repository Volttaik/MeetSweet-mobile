...
function normalizeComment(raw: any, commentRoomId: string): CommentRoomComment {
  return {
    ...
    parentId: raw.parent_id ?? raw.parentId ?? raw.parent_comment_id ?? raw.parentCommentId ?? null,
    body: raw.body ?? '',
 ...
 * destination is commentRoomId. Optional parentCommentId for threaded replies.
 * POST /api/comment-rooms/:commentRoomId/comments
 */
export async function submitRoomComment(
  ...
  if (opts?.parentCommentId) {
    payload.parent_id = opts.parentCommentId;
    payload.parentCommentId = opts.parentCommentId;
  }
...
/**
 * GET replies for a parent comment in a Comment Room.
 * GET /api/comment-rooms/:commentRoomId/comments/:commentId/replies
 */
export async function getRoomCommentReplies(
  commentRoomId: string,
  commentId: string,
): Promise<{ replies: CommentRoomComment[] }> {
  const token = await getToken();
  const headers: Record<string, string> = token ? authHeader(token) : {};
  try {
    const raw = await apiFetch<{ replies?: unknown[]; comments?: unknown[] }>(
      `/comment-rooms/${encodeURIComponent(commentRoomId)}/comments/${encodeURIComponent(commentId)}/replies`,
      { headers },
    );
    const list = raw?.replies ?? raw?.comments ?? [];
    return {
      replies: Array.isArray(list) ? list.map((c) => normalizeComment(c, commentRoomId)) : [],
    };
  } catch {
